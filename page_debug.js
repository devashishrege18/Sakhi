...
// Mock Firebase functions to unblock build (as requested by user to fix build)
const saveChat = async (id, title, messages) => {
  if (typeof window !== 'undefined') localStorage.setItem(`chat_${id}`, JSON.stringify({ id, title, messages }));
};
const getChats = async () => {
  if (typeof window === 'undefined') return [];
  return Object.keys(localStorage).filter(k => k.startsWith('chat_')).map(k => JSON.parse(localStorage.getItem(k)));
};
const deleteChat = async (id) => {
  if (typeof window !== 'undefined') localStorage.removeItem(`chat_${id}`);
};
...
const INDIAN_LANGUAGES = [
  ...
  { code: 'pa-IN', name: '\u0A2A\u0A70\u0A1C\u0A2C\u0A40', english: 'Punjabi' },
...
export default function Home() {
  ...
  // --- CORE FUNCTIONS (Moved Up to avoid Hoisting Issues) ---
  ...
  const handleVoiceCommand = async (text) => {
    ...
    // Check keywords in INPUT first
    for (const [key, data] of Object.entries(VOICE_FEATURES)) {
      if (data.keywords.some(k => lower.includes(k))) {
        intentRoute = data.route;
        break;
      }
    }
    ...
    try {
      ...
      // Check AI response for override keywords if intent wasn't found in input
      if (!intentRoute) {
    ...
    // Execute Redirect (Independent of API success)
    if (intentRoute) {
      setTimeout(() => {
        window.location.href = intentRoute;
      }, 2000);
    } else {
      // Fallback DOM check for safety
      setTimeout(() => {
        try {
          const boxes = document.querySelectorAll('.message-row.assistant');
          if (boxes.length > 0) {
            const txt = boxes[boxes.length - 1].textContent.toLowerCase();
            for (const [feature, data] of Object.entries(VOICE_FEATURES)) {
              if (data.keywords.some(k => txt.includes(k))) {
  ...
  const initSpeechRecognition = (langCode) => {
    if (typeof window !== 'undefined' && ('SpeechRecognition' in window || 'webkitSpeechRecognition' in window)) {
      ...
      recognitionRef.current.onresult = (e) => {
        ...
        setInput(t);
        // Only process if FINAL result to avoid premature cutoffs
  ...
  // --- USE EFFECTS (Now safe to call functions) ---
  ...
  // Re-init speech when language changes
  useEffect(() => {
    // Only re-init if we were already listening or just to update config? 
    // Safest is to just update the ref if it exists, or let toggleListening handle it. 
